...
# This script uses HotpotQA context fields as a static candidate pool.
# It is a minimal RAG-style setup, not a full retriever pipeline.
...
def featurize_passages(query, passages):
...
def parse_hotpot_example(example, max_passages):
  ...
  return {
      ...
      'answer': answer,
      'passages': passages,
      'features': featurize_passages(question, passages),
  }


def load_hotpot_subset(max_passages, train_examples, val_examples, seed):
  raw = load_dataset('hotpot_qa', 'distractor', split='train')
  raw = raw.shuffle(seed=seed)
  parsed = []
  for ex in raw:
    parsed_ex = parse_hotpot_example(ex, max_passages=max_passages)
...
def init_scorer(input_dim, hidden_units, seed):
...
def main():
  ...
  scorer_pg = init_scorer(input_dim=train_examples[0]['features'].shape[1], hidden_units=[32, 32], seed=args.seed)
  scorer_pl = init_scorer(input_dim=train_examples[0]['features'].shape[1], hidden_units=[32, 32], seed=args.seed)
